...
const HomePage = () => {
  ...
  useEffect(() => {
    const getAllTransactions = async () => {
      try {
        ...
        setLoading(true);
        const res = await axios.post(`${SERVER_URL}/transactions/get-transaction`, { userid: user._id, frequency, selectedDate, type });
        setLoading(false);
  ...
  const handleDelete = async (record) => {
    try {
      setLoading(true);
      await axios.post(`${SERVER_URL}/transactions/delete-transaction`, { transactionId: record._id });
      setLoading(false);
  ...
  const handleSubmit = async (value) => {
    try {
      ...
      setLoading(true);
      if (editable) {
        await axios.post(`${SERVER_URL}/transactions/edit-transaction`, { payload: { ...value, userid: user._id }, transactionId: editable._id });
        setLoading(false);
        window.location.reload();
        message.success('Transaction Edited Successfully');
      } else {
        await axios.post(`${SERVER_URL}/transactions/add-transaction`, { ...value, userid: user._id });
        setLoading(false);
        window.location.reload();
    ...
    <Layout>
      ...
      <div className='Main-filter'>
            <div className='filters'>
                ...
                <UnorderedListOutlined className={`mx-2 ${viewData === 'table' ? 'active-icon' : 'inactive-icon'}`} onClick={(values) => setViewData('table')} />
                <AreaChartOutlined className={`mx-2 ${viewData === 'analytics' ? 'active-icon' : 'inactive-icon'}`} onClick={(values) => setViewData('analytics')} />
                </div>
                <div>
                <button className='btn btn-primary add-new-btn' onClick={() => setShowModal(true)}>Add New</button>
                <button className='btn btn-primary add-new-float-btn' onClick={() => setShowModal(true)}>
                    <PlusOutlined />
                </button>
                </div>
            </div>
